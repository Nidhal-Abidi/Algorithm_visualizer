...
const linearSearch = (
  ...
  if (index < arrNbrs.length) {
    rectToCircleAnimation(ctx, x_axis_pos, y_axis_pos, edge, arrNbrs[index])
    ...
    if (x_axis_pos + 2 * (edge + dx) < canvas.width) {
      setTimeout(() => {
        circleToRectAnimation(ctx, x_axis_pos, y_axis_pos, edge, arrNbrs[index])
        linearSearch(
          arrNbrs,
          key,
          x_axis_pos + edge + dx,
